class Game {
  ...
  _removePiece(piece) {
    ...
    this.playerPieces[piece.color].splice(
      this.playerPieces[piece.color].indexOf(piece),
      1
    );
  }
  ...
  undo() {
    ...
    for (const subStep of step) {
      ...
      if (
        subStep.from !== 0 &&
        subStep.to !== 0 &&
        (!subStep.castling || subStep.piece.rank === "king")
      ) {
        this.softChangeTurn();
      }
    }
  ...
  unblockedPositions(piece, allowedPositions, checking = true) {
    ...
    if (piece.rank === "pawn") {
      ...
      for (const move of allowedPositions[1]) {
        //moving moves
        if (
          myBlockedPositions.indexOf(move) !== -1 ||
          otherBlockedPositions.indexOf(move) !== -1
        ) {
          break;
